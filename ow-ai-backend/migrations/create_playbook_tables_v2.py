"""
🏢 ENTERPRISE MIGRATION: Automation Playbooks System v2
Properly handles existing users table foreign keys
...
from sqlalchemy import create_engine, text, MetaData, Table, Column, String, Integer, Float, Boolean, DateTime, Text, ForeignKey, CheckConstraint
...
def create_playbook_tables():
    """Create automation playbook tables with proper foreign keys"""
    
    print("🏢 ENTERPRISE MIGRATION: Automation Playbooks System v2")
    print("=" * 70)
    ...
    try:
        ...
        # ENTERPRISE FIX: Reflect existing users table so foreign keys work
        print("🔄 Loading existing schema...")
        users_table = Table('users', metadata, autoload_with=engine)
        print(f"  ✅ Reflected users table ({len(users_table.columns)} columns)")
        print("")
        
        print("🔄 Creating automation playbook tables...")
        print("")
        ...
        # Create tables
        metadata.create_all(engine, tables=[automation_playbooks, playbook_executions], checkfirst=True)
        ...
        with engine.begin() as conn:
            ...
            if admin_id:
                ...
                print("  ⚠️  Admin user not found, using NULL for created_by")
            ...
            conn.execute(text("""
                INSERT INTO automation_playbooks (
                    ...
                    :id, :name, :description, :status, :risk_level, :approval_required,
                    :trigger_conditions::jsonb, :actions::jsonb, :last_executed, :execution_count,
                    :success_rate, :created_by, :created_at, :updated_at
                )
        ...
        with engine.connect() as conn:
            ...
            # Show the playbook
            result = conn.execute(text("SELECT id, name, status, risk_level, execution_count, success_rate FROM automation_playbooks"))
            playbooks = result.fetchall()
            if playbooks:
                print("")
                print("📋 Playbooks in database:")
                for pb in playbooks:
                    print(f"  • {pb[1]} (ID: {pb[0]}, Status: {pb[2]}, Risk: {pb[3]}, Executions: {pb[4]}, Success: {pb[5]}%)")
