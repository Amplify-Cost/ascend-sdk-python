          ...
          <div className="space-y-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border-l-4 border-green-500 p-8 shadow-md">
            <div className="flex items-center justify-between mb-4">
              ...
              {realTimeMetrics.data_quality && (
                <span className={`px-4 py-2 rounded-full text-sm font-medium ${
                  ...
                  {realTimeMetrics.real_time_overview?.status?.message || 'Loading...'}
                </span>
              )}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <MetricCard
                ...
                subtitle={realTimeMetrics.real_time_overview?.status?.has_data ? "Current users" : "No activity detected"}
              />
              <MetricCard
                icon={Shield}
                ...
                subtitle="Last hour"
                trend={(realTimeMetrics.real_time_overview?.recent_high_risk_actions || 0) > 5 ? '+Alert' : (realTimeMetrics.real_time_overview?.status?.has_data ? 'Normal' : 'No data')}
              />
              <MetricCard
                icon={Activity}
                title="Active Agents"
                value={realTimeMetrics.real_time_overview?.active_agents || 0}
                subtitle={realTimeMetrics.real_time_overview?.status?.has_data ? "Currently running" : "Awaiting activity"}
              />
              <MetricCard
                icon={BarChart3}
                title="Total Actions"
                value={realTimeMetrics.real_time_overview?.total_actions_last_hour || 0}
                subtitle="Last hour"
                trend={realTimeMetrics.real_time_overview?.status?.has_data ? 'Active' : 'Idle'}
              />
            </div>
