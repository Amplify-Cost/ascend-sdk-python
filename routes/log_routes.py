...
def get_logs(
    ...
    """SEC-082: Multi-tenant log access with organization filtering"""
    return db.query(AgentAction).filter(
        AgentAction.organization_id == org_id
    ).order_by(AgentAction.timestamp.desc()).limit(100).all()
...
def get_trends(
    ...
    """SEC-082: Multi-tenant analytics with organization filtering"""
    high_risk_daily = (
        db.query(
