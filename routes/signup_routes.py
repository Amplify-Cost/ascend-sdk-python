...
async def create_organization_from_signup(signup_id: int, db: Session):
    ...
    try:
        ...
        # Provision Cognito pool
        try:
            provisioner = CognitoPoolProvisioner()
            pool_config = await provisioner.create_organization_pool(organization)
            ...
            logger.error(f"SEC-021: Failed to create Cognito pool for org {organization.id}: {e}")
