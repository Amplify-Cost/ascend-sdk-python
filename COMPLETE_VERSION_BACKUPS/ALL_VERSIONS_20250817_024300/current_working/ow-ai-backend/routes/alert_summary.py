...
async def summarize_alerts(
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            raise ValueError(
                "Invalid data format. Expected array of alerts or object with 'alerts' field"
            )

        logger.info("Combined text length: %s", len(combined_text))
        ...
        primary_action_type = next(iter(action_types), "security_alert")

        logger.info(
            "Calling generate_summary with agent_id: %s, action_type: %s",
            primary_agent_id,
            primary_action_type,
        )
...
async def summarize_alert_text(
    ...
    try:
        ...
        logger.error("Error in summary-text endpoint: %s", str(e))
